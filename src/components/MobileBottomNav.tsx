...
import { LayoutGrid, MapPin, Heart, MessageCircle, User as UserIcon } from 'lucide-react';
...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-slate-950/95 backdrop-blur-xl border-t border-slate-800/80 px-4 py-2">
      <div className="max-w-md mx-auto flex items-center justify-between relative">
        ...
        >
          ...
          <span className="text-[10px] mt-1 tracking-tight">Nearby</span>
        </button>

        {/* Center Floating Action Button: Likes / Main Action */}
        <button
          onClick={() => setActiveTab('likes')}
          className="relative -mt-6 p-1 rounded-full bg-slate-950 transition-transform active:scale-95"
          title="Likes & Matches"
        >
          <div className="w-13 h-13 sm:w-14 sm:h-14 rounded-full bg-gradient-to-tr from-rose-500 via-pink-500 to-purple-600 flex items-center justify-center shadow-lg shadow-pink-500/40 hover:shadow-pink-500/60 transition-shadow p-3">
        ...
        <button
          ...
          className={`relative flex flex-col items-center py-1 px-2 transition-all ${
